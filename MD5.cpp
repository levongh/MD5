...
{
    return update((uint8_t*)input, inputLen);
}

MD5& MD5::update(const unsigned char* input, size_t inputLen)
{
    auto index = static_cast<uint32_t>((m_low >> 3) & 0x3F);

    if ((m_low += ((uint32_t)inputLen << 3)) < ((uint32_t)inputLen << 3)) {
        m_high++;
    }
    m_high += ((uint32_t)inputLen >> 29);
...
MD5& MD5::finalize()
{
    ...
    if (!m_finalized) {
        uint8_t bits[8];
        memcpy(bits, &m_low, 4);
        memcpy(bits + 4, &m_high, 4);

        size_t index = (uint32_t)((m_low >> 3) & 0x3f);
...
std::string MD5::toString() const
{
    ...
    char result[33] = {0};
    for (int i = 0; i < 16; ++i) {
        sprintf(result + i * 2, "%02x", m_digest[i]);
    }

    return std::string(result);
}

Hash128 MD5::shortened()
{
    if (!m_finalized) {
        return Hash128();
    }
    return Hash128(&m_digest[0], &m_digest[16]);
}
